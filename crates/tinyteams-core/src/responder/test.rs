...
#[test]
fn duplicate_candidate_detail_is_a_structural_error() {
    let members = [member("alice", None)];
    let roster = Roster::new(&members, &[], &[]);
    let desks = DeskSet::new(&[], &[], &[], &[], &[]);
    ...
    assert_eq!(
        responder_plan(&request(None), &roster, &desks, &[detail.clone(), detail]),
...
#[test]
fn responder_payload_wire_form_is_stable() {
    let value = serde_json::to_value(ResponderDecision {
        responder_id: "alice".into(),
    ...
    assert_eq!(
        value,
        serde_json::json!({"responder_id":"alice","rung":"auto_selection","disposition":"selected"})
    );
}
