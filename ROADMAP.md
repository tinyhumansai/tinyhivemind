...
| P21 | Concurrent rounds: a step authorizes a bounded *round* of turns rather than one, `next_state` moves onto the round, and a peer row written in the same round is invisible to it — depth becomes rounds rather than turns | **in progress** — see [`docs/specs/concurrent-rounds.md`](docs/specs/concurrent-rounds.md) and [ADR 0014](docs/adr/0014-a-round-authorizes-concurrent-turns.md) |
