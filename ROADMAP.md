...
| P19 | Folding by size: the standing account triggers on the characters of foldable content as well as its row count, stated by a host as a token budget, so a seat joining a large room always has an account of it | planned, **not implemented** — see [`docs/specs/folding-by-size.md`](docs/specs/folding-by-size.md) |
| P20 | A real provider layer for the `desk` example: `tinyinference` behind the `Digester` and wrap-up paths in place of a `curl` subprocess, and `tinytools` rendering the room's tool surface — both example-only dev-dependencies, no library crate touched | planned, **not implemented** — see [ADR 0013](docs/adr/0013-a-vendored-crate-is-an-example-dependency.md) |
