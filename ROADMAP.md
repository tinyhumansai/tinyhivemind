...
| P24 | The seat-per-facet shape as the default: a bounded, pure fold that splits a task's facets across seats and pools each seat's peers on its own facet, so a host gets `hive+fold` from the library rather than from the benchmark — with the on-floor room kept as the arm it beat | **planned**, not implemented — the shape is measured in [the experiment](docs/experiments/2026-09-09-variety-and-roles.md), the library still only offers the floor |
