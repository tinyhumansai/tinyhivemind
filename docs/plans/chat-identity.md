...
`src/server/chat_history.rs` into `tinyhivemind-core::chat`, byte-for-byte
...
**Files:** `crates/tinyhivemind-core/src/chat/mod.rs`,
`crates/tinyhivemind-core/src/chat/test.rs`
...
3. Run `cargo test -p tinyhivemind-core chat`.
...
`crates/tinyhivemind-core/examples/basic.rs`, `ROADMAP.md`
...
**Files:** `crates/tinyhivemind-core/src/greeting/`,
`crates/tinyhivemind-core/src/error/`, `crates/tinyhivemind-core/Cargo.toml`
...
- [x] `cargo run -p tinyhivemind-core --example basic`
...
PR bumps `vendor/tinyhivemind` and switches its own `chat_history.rs` call sites
to `tinyhivemind_core::chat`, pinning `GENERAL_DESK` against
